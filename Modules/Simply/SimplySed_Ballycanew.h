...
This is a simple sediment transport module created as a part of SimplyP.
	...
	auto ExhaustionLag = RegisterParameterDouble(Model, Sediment, "Sediment source exhaustion lag", Dimensionless, 0.0);
	auto ExhaustionScale = RegisterParameterDouble(Model, Sediment, "Sediment source exhaustion scale", Dimensionless, 1.0);
	
	auto SedimentExhaustion = RegisterEquation(Model, "Sediment exhaustion", Dimensionless);
	auto SedimentExhaustionFactor = RegisterEquation(Model, "Sediment exhaustion factor", Dimensionless);
	
	EQUATION(Model, SedimentExhaustion,
		return PARAMETER(ExhaustionLag)*LAST_RESULT(SedimentExhaustion) + PARAMETER(ExhaustionScale)*LAST_RESULT(ReachSedimentInput);
	)
	
	EQUATION(Model, SedimentExhaustionFactor,
		double x = RESULT(SedimentExhaustion);
	...
	EQUATION(Model, ReachSedimentInput,
		return RESULT(TotalReachSedimentInputCoefficient) * RESULT(ErosionFactor) * RESULT(SedimentExhaustionFactor);
	)

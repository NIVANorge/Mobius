...
\item Edit indexes (ctrl-E). You can edit what indexes are in each index set. If you add a nex index, the parameters indexed by that index will be given default values. Note that you can currently not set branch connectivity of branched index sets here. This still has to be done in the parameter file (for now). Moreover, removing indexes from branched index sets may not work correctly.

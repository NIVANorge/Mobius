...
Usually you will have one or more header (.h) files with the procedures you use to build your modules, and one source file (.cpp) with the main C++ program that composes the modules into a model. The below example shows how to set up a model .dll that can be accessed through MobiView or the python wrapper. It is very convenient to use MobiView during model development to visualize how new processes are behaving (but remember to recompile every time). It is also possible to make a command line application (.exe) using the model interaction API described in section \ref{sec:modelinteraction}.
...
The {\tt mobius\_model} object contains lists of various model entities (see Table \ref{tab:modelentity} for an explanation) and information about these. It is the model developer's job to register all the entities and provide the needed information for each one.
...
\hline
\end{tabular}
\caption{SimplySnow parameters and state variables}
\end{table}
...
\begin{example}
Registration of model entities for the SimplySnow module.
...
Equation bodies for the SimplySnow module.
\begin{lstlisting}[style=mycpp]
...
\begin{example}
Batch structure for the SimplySnow module if the user provided precipitation and air temperature timeseries per reach.
\begin{lstlisting}[style=textstyle]
[Reaches]
        -----
        Precipitation falling as snow
...
You can turn the debug flags either on or off by setting them to 1 or 0 respectively (or turn them off by removing them entirely). All of them will make the model run a bit slower in total, so they should probably be turned off when you are creating a release version of the model.
...
\item {\tt MOBIUS\_TEST\_FOR\_NAN}. If turned on, this will test every result value to see if it was an inf or nan. In the case of such a value, it will halt the model execution and print out the current timestep, the state of the index sets, and the state of all the parameter, input and result dependencies of the equation that produced the illegal value. The nan test will not work correctly if you compile with the -ffast-math flag. The -ffast-math flag may make the model run a little faster, but the underlying floating point architecture will no longer make nans detectable, which can interfer both with this safeguard and other safeguards you may put in.
...
\item {\tt MOBIUS\_EQUATION\_PROFILING}. This will give you very fine-grained information about how many processor ticks every equation evaluation took on average, and how many times each equation was evaluated. Note that the time to evaluate an equation is not always the same as the time to produce a result value since a solver may evaluate an equation many times to produce a result value for it. See Section \ref{sec:performance} about tips on how to improve the speed of the model.
\item {\tt MOBIUS\_INDEX\_BOUNDS\_TESTS}. This should probably be turned on as often as possible during model development. It will catch if during explicit indexing you provided an {\tt index\_t} that was out of bounds for this index set. It will also give you an error if you use it to index a different index set than it was created from.
\end{enumerate}
...
\apipar{bool UseEndDate}{(optional) Set this to true if you want an "End date" parameter instead of a "Timesteps" parameter.}
\apipar{const char *TimestepSize}{(optional) You can provide the timestep resolution here instead of calling {\tt SetTimestepSize} later. Default is 1 day.}
...
\apientry{ReadInputDependenciesFromFile}{Model interaction procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
...
\apientry{GenerateDataSet}{Model interaction procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
...
\apientry{ReadParametersFromFile}{Model interaction procedure}{
...
\apientry{ReadInputsFromFile}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to the dataset object.}
...
\apientry{CopyDataSet}{Model interaction procedure}{
...
\apientry{SetIndexes}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{SetParameterValue}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{GetParameter{\emph X}}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
\apipar{const char *Name}{The name of a parameter in the model.}
\apipar{const std::vector<const char *>\& Indexes}{A list of index names, each index corresponding to one of the index sets the parameter indexes over. These have to be in the right order.}
...
\apientry{SetInputSeries}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{GetResultSeries}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{GetInputSeries}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{PrintResultSeries or PrintInputSeries}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
\apipar{const char *Name}{The name of a result (equation) of the model.}
\apipar{const std::vector<const char *>\& Indexes}{A list of index names, each index corresponding to one of the index sets the result indexes over. These have to be in the right order.}
\apipar{size\_t WriteSize}{How many values are requested.}
...
\apientry{PrintIndexes}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{PrintResultStructure}{Model interaction procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object}
...
\apientry{PrintParameterStorageStructure}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
\apidesc{Prints the storage structure of the parameters in the model to the standard output.}
}

\apientry{PrintInputrStorageStructure}{Model interaction procedure}{
\apipar{mobius\_data\_set *DataSet}{Pointer to a dataset object.}
...
\apientry{BeginModule}{Registration procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
\apipar{const char *Name}{The name of the module.}
\apipar{const char *Version}{The current version of the module.}
\apiret{A {\tt module\_h} handle to the newly registered module.}
\apidesc{Every parameter group and equation that is registered between this call and an {\tt EndModule} call is considered a part of this module.}
}

\apientry{EndModule}{Registration procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
\apidesc{Closes the current module. If you need to you can reopen it later using the same name.}
}

\apientry{SetModuleDescription}{Registration procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
\apipar{const char *Description}{A description of the module in the MarkDown format.}
\apidesc{Sets a description that can be viewed from the model info window in MobiView.}
}
...
\apipar{index\_set\_h IndexSets...}{(0 or more arguments) Handles to index sets that the group should index over.}
...
\apipar{const char *Symbol}{(optional) A short symbol to be used to refer to the parameter in the autocalibration and sensitivity analysis in the python wrapper and MobiView.}
...
\apientry{RegisterParameterEnum}{Registration procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
\apipar{parameter\_group\_h ParameterGroup}{Handle to the parameter group that the parameter should be put in.}
\apipar{const char *Name}{The full name of the parameter.}
\apipar{std::initializer\_list<const char *> \&EnumNames}{List of the possible values of the parameter.}
\apipar{const char *Default}{The default value of the parameter. Must be one of the values in the list.}
\apipar{const char *Description}{(optional) A long-form description of the parameter.}
\apipar{const char *Symbol}{(optional) A short symbol to be used to refer to the parameter in the autocalibration and sensitivity analysis in the python wrapper and MobiView.}
\apiret{A {\tt parameter\_enum\_h} handle to the newly registered parameter.}
\apidesc{Register a new parameter of type enum with the model.}
}

\apientry{EnumValue}{Registration procedure}{
\apipar{mobius\_model *Model}{Pointer to the model object.}
\apipar{parameter\_enum\_h ParameterHandle}{Handle to a registered Enum parameter.}
\apipar{const char *StringValue}{The string value of one of the possible values of the Enum.}
\apiret{A {\tt u64} integer value of the enum corresponding to the string value.}
\apidesc{This gives you an integer value that can be used inside equations to compare against what is returned by {\tt PARAMETER(ParameterHandle)}.}
}
...
\apipar{solver\_h Solver}{(optional). A handle to a solver to put this equation on.}
...
SetEquation(Model, Handle,
	[=](model_run_state *RunState__) -> double {
		Decl
	}
);
\end{SaveVerbatim}
...
\multicolumn{2}{|z|}{where {\tt Decl} stands for possibly multiple lines of code of an equation body. These lines of code always have to return a {\tt double}. The {\tt model\_run\_state} is an object that can be used by other macros (such as {\tt PARAMETER} or {\tt RESULT}) inside the {\tt Decl} to extract values.}\\
}

The following macros are only available inside the {\tt Decl} of an {\tt EQUATION}. This is because they expand to something that refer to the {\tt model\_run\_state} that is given as an argument in the lambda inside the expansion of the {\tt EQUATION} macro.
...
\apidesc{Returns the value of the parameter, depending on the current state of the index sets. This is a macro that expands to an expression which either extracts a value from the {\tt model\_run\_state} or registers a dependency of the surrounding {\tt Equation} on this parameter, depending on the context of the evaluation.}
}

\apientry{INPUT}{Equation macro}{
\apipar{input\_h InputHandle}{Handle to an input that you wish to extract the value of.}
\apiret{A {\tt double}.}
\apidesc{Returns the current timestep value of the input timeseries, depending on the current state of the index sets. This is a macro that expands to an expression which either extracts a value from the {\tt model\_run\_state} or registers a dependency of the surrounding {\tt Equation} on this input, depending on the context of the evaluation.}
}
...
\apipar{index\_t Indexes\ldots}{(optional, varargs) One or more indexes to explicitly index.}
\apiret{A {\tt double}.}
\apidesc{Returns the value of the current timestep result of the equation, depending on the current state of the index sets. This is a macro that expands to an expression which either extracts a value from the {\tt model\_run\_state} or registers a dependency of the surrounding {\tt Equation} on this equation, depending on the context of the evaluation.}
}
...
\apientry{CURRENT\_TIMESTEP}{Equation macro}{
\apipar{}{None.}
\apiret{A {\tt s64}.}
\apidesc{Returns what timestep the simulation is on. -1 corresponds to the "intial value" timestep, while 0 is the first proper timestep of the simulation. This is a macro that expands to an expression that extracts the value from the {\tt model\_run\_state}.}
}

\apientry{INDEX\_COUNT}{Equation macro}{
\apipar{index\_set\_h IndexSet}{Handle to an index set.}
\apiret{A {\tt size\_t}.}
\apidesc{Returns the amount of indexes in this index set. This is a macro that expands to an expression that extracts the value from the {\tt model\_run\_state}.}
}

\apientry{CURRENT\_INDEX}{Equation macro}{
\apipar{index\_set\_h IndexSet}{Handle to an index set.}
\apiret{A {\tt index\_t}.}
\apidesc{Returns the current (numeric) index of the given index set. This is a macro that expands to an expression that either extracts the value from the {\tt model\_run\_state} or registers a dependency of the surrounding {\tt Equation} on this index set, depending on the contex of the evaluation.}
}

\apientry{BRANCH\_INPUTS}{Equation macro}{
\apipar{index\_set\_h IndexSet}{Handle to a branched index set.}
\apidesc{Retuns a struc that allows you to iterate over input branches of a branched index set. Note that {\tt BRANCH\_INPUTS} calls {\tt CURRENT\_INDEX(IndexSet)}, and thus registers an index set dependency.}\\
}

\apientry{INPUT\_COUNT}{Equation macro}{
\apipar{index\_set\_h IndexSet}{Handle to a branched index set.}
\apiret{A {\tt size\_t}.}
\apidesc{Returns the amount of branch input indexes to the current index in the index set. This is a macro that expands to an expression that either extracts the value from the {\tt model\_run\_state} or registers a dependency of the surrounding {\tt Equation} on this index set, depending on the contex of the evaluation.}
}

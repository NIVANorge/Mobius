...
\item {\tt CURRENT\_DAY\_OF\_YEAR()} Returns an integer telling you which day of year it is (in the current model state). January 1st is always day 1. For instance, if the model "Start date" is 1999-1-15, then during the first timestep, the current day of year will be 15.
\item {\tt DAYS\_THIS\_YEAR()} Returns an integer that is either 365 (in a normal year) or 366 (in a leap year).

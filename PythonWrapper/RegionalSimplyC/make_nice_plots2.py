...
simmed   = ('Reach flow (daily mean, cumecs)', ['River'])
observed = ('Observed flow', [])

simmeddoc = ('DOC flux from reach, daily mean', ['River'])
...
def main() :
	...
	fig_month, ax_month = plt.subplots(6, 6)
	fig_month.set_size_inches(20, 30)
	
	ax_month = ax_month.flatten()
	
	fig_year, ax_year = plt.subplots(6, 6)
	...
	for index, row in catch_setup.iterrows():
		...
		if row['reduced_set'] != 'y' : continue
		#if catch_name != 'Dalelva' : continue
		
		param_file_prefix = 'optim_DOC_1lu_'
		#param_file_prefix = 'norm3_optim_params_DOC'
		
		infile  = 'MobiusFiles/inputs_%d_%s.dat' % (catch_no, catch_name)
		parfile = 'MobiusFiles/OptimResults/%s_%d_%s.dat' % (param_file_prefix, catch_no, catch_name)
		...
		add_normalized_plots(ax_month[2*plotno], month_x, obs, sim, '#0082C8', '#E6194B')
		add_normalized_plots(ax_month[2*plotno+1], month_x, fluxobs, fluxsim, '#3CB44B', '#F58230')
		
		ax_month[2*plotno].set_title('%s Q' % fullname)
		ax_month[2*plotno].legend()
		ax_month[2*plotno].set_xticks(month_x)
		ax_month[2*plotno].axhline(0, color='grey')
		
		ax_month[2*plotno+1].set_title('%s DOC flux' % fullname)
		ax_month[2*plotno+1].legend()
		ax_month[2*plotno+1].set_xticks(month_x)
		ax_month[2*plotno+1].axhline(0, color='grey')
		...
		add_normalized_plots(ax_year[2*plotno], year_x, df_year[obsname].values, df_year[simname].values, '#0082C8', '#E6194B')
		
		add_normalized_plots(ax_year[2*plotno+1], year_x, df_year[obsfluxname].values, df_year[simdocname].values, '#3CB44B', '#F58230')
		
		ax_year[2*plotno].set_title('%s Q' % fullname)
		ax_year[2*plotno].legend()
		#ax[0].set_xticks(year_x)
		ax_year[2*plotno].axhline(0, color='grey')
		
		ax_year[2*plotno+1].set_title('%s DOC flux' % fullname)
		ax_year[2*plotno+1].legend()
		#ax[1].set_xticks(year_x)
		ax_year[2*plotno+1].axhline(0, color='grey')
